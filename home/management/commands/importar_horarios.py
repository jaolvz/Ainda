...
from home.models import HorarioParada
import pandas as pd

class Command(BaseCommand):
    help = 'Importando horários_parada via CSV'
    ...
    def handle(self, *args, **kwargs):
        ...
        for chunk in pd.read_csv(csv_file, chunksize=chunksize):
            ...
            for index, row in chunk.iterrows():
                instance = HorarioParada(
                    rota=row['trip_id'],
                    numero_parada=row['stop_sequence'],
                    parada=row['stop_id'],
                    horario=row['arrival_time']
                )
                bulk_instances.append(instance)

            # Inserir instâncias em massa no banco de dados
            HorarioParada.objects.bulk_create(bulk_instances)
